...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .vol-toolbar { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }
  .vol-search-wrap { display: flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 7px 12px; flex: 1; min-width: 200px; max-width: 320px; }
  .vol-search-wrap input { background: transparent; border: none; outline: none; color: var(--text); font-family: var(--font-body); font-size: 13px; width: 100%; }
  .vol-search-wrap input::placeholder { color: var(--muted); }
  .vol-filter-btn { display: flex; align-items: center; gap: 6px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; color: var(--muted); font-family: var(--font-body); font-size: 13px; padding: 7px 14px; cursor: pointer; transition: all .2s; white-space: nowrap; }
  ...
  .vol-skill-input-wrap { display: flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 7px 12px; }
  .vol-skill-input-wrap input { background: transparent; border: none; outline: none; color: var(--text); font-family: var(--font-body); font-size: 13px; width: 140px; }
  .vol-skill-input-wrap input::placeholder { color: var(--muted); }
  .vol-skill-search-btn { background: var(--accent); border: none; border-radius: 6px; color: #0d1117; font-family: var(--font-body); font-size: 12px; font-weight: 500; padding: 4px 10px; cursor: pointer; transition: opacity .2s; white-space: nowrap; }
  ...
  .vol-stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 16px 20px; display: flex; flex-direction: column; gap: 4px; }
  ...
  .dash-id { font-family: monospace; font-size: 11px; color: var(--muted); max-width: 60px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: block; }
  ...
  .skill-tag { background: rgba(245,166,35,.1); color: var(--accent2); font-size: 11px; font-weight: 500; padding: 2px 8px; border-radius: 12px; border: 1px solid rgba(245,166,35,.2); }
  .toggle-btn { display: flex; align-items: center; gap: 6px; background: transparent; border: 1px solid var(--border); border-radius: 7px; color: var(--muted); font-family: var(--font-body); font-size: 12px; padding: 5px 10px; cursor: pointer; transition: all .2s; }
  .toggle-btn:hover { border-color: var(--accent); color: var(--text); }
  .vol-status { display: inline-flex; align-items: center; font-size: 12px; font-weight: 500; }
  .vol-status.available { color: #3ecf5a; }
  .vol-status.unavailable { color: var(--muted); }
  .vol-add-btn { display: flex; align-items: center; gap: 6px; background: var(--accent); border: none; border-radius: 8px; color: #0d1117; font-family: var(--font-body); font-size: 13px; font-weight: 500; padding: 8px 16px; cursor: pointer; transition: opacity .2s; }
  ...
  .modal-box { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; width: 100%; max-width: 520px; max-height: 90vh; overflow-y: auto; animation: modalIn .2s ease; }
  ...
  .skill-select-wrap { display: flex; flex-wrap: wrap; gap: 6px; background: var(--surface2); border: 1px solid var(--border); border-radius: 8px; padding: 8px 10px; min-height: 42px; }
  .skill-chip { display: inline-flex; align-items: center; gap: 4px; font-size: 12px; font-weight: 500; padding: 3px 10px; border-radius: 20px; border: 1px solid var(--border); cursor: pointer; transition: all .15s; color: var(--muted); background: transparent; user-select: none; }
...
export default function Volunteers() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setMessage(err.response?.data?.message || "Gönüllüler getirilemedi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAvailable = async () => {
    ...
    try {
      ...
      setMessage(
        err.response?.data?.message || "Müsait gönüllüler getirilemedi.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchBySkill = async () => {
    ...
    try {
      ...
      setMessage(err.response?.data?.message || "Yetenek ile arama başarısız.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    setSubmitting(true);
    try {
      await axios.post("http://localhost:8080/api/volunteer", {
      ...
      setMessage(err.response?.data?.message || "Gönüllü oluşturulamadı.");
      setShowModal(false);
    } finally {
      setSubmitting(false);
    }
        ...
        <main className="al-main">
          ...
          <div className="vol-stats">
            ...
            <div className="vol-stat-card">
              ...
              <div className="vol-stat-value orange">
